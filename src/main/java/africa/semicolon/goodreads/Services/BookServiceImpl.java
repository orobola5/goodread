...
public class BookServiceImpl implements BookService{
    ...
    @Override
    public Book findBookByTitle(String title) throws GoodReadException, GoodReadException {
        Book book = bookRepository.findBookByTitleIsIgnoreCase(title);
        if (book == null){
            throw new GoodReadException("Book not found", 404);
        }
        return book;
    ...
    @Override
    public Map<String, String> generateDownloadUrls(String fileName, String imageFileName) throws GoodReadException {
        if (!amazonS3.doesObjectExist(FILE_BUCKET, fileName)){
            throw new GoodReadException("File does not exist", 400);
        }
